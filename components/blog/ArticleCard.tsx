  ...
  return (
    ...
    >
      <Link href={`/blog/${articulo.slug}`} className="block h-full no-underline group">
        <article
          className="card-glow relative h-full flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm hover:shadow-md transition-all duration-500 hover:-translate-y-1"
          style={{ border: '1px solid var(--blog-card-border)' }}
        >
          {/* Image */}
          <div className="relative aspect-[16/10] overflow-hidden"
               style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--blog-accent-blue) 10%, transparent), #f1f5f9)' }}>
            {showImage ? (
              <>
                <img
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                <span className="absolute text-black/10 text-6xl"
            ...
            {articulo.categoria && (
              <div className="absolute top-4 left-4">
                <span className="inline-flex items-center px-3 py-1.5 rounded-full bg-white/80 backdrop-blur-md border border-black/10 text-black/80 text-[10px] tracking-[0.18em] uppercase"
            ...
            <div className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/80 backdrop-blur-md border border-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 group-hover:scale-110">
              <ArrowUpRight className="w-4 h-4" style={{ color: 'var(--blog-accent)' }} />
            </div>
          </div>

          {/* Content */}
          <div className="p-6 flex flex-col flex-1">
            <div className="text-[11px] tracking-[0.18em] text-black/35 uppercase mb-3"
                 style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 300 }}>
              {formatDate(articulo.fecha_publicacion)}
            </div>
            <h3
              className="text-xl md:text-[1.35rem] text-black leading-snug tracking-tight mb-3 transition-colors duration-300"
              style={{
            ...
            {articulo.meta_description && (
              <p className="text-sm text-black/55 leading-relaxed line-clamp-3 flex-1"
                 style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 300 }}>
                {articulo.meta_description}
              </p>
            )}
            <div className="mt-5 pt-5 border-t border-black/8 flex items-center justify-between">
              ...
              <div className="w-7 h-px bg-black/10 transition-all duration-500" />
            </div>
          </div>
        </article>
      </Link>
